...
def start_production(conn, product_id, quantity, start_date, notes=None, variant_id=None):
    ...
    try:
        ...
        logger.error(f"Erro ao iniciar produção: {e}")
        raise

def move_stage(conn, item_id, current_stage, next_stage, qty_move, total_qty, selected_variant_id=None, deduct_glaze=False, user_id=None, username='WIP'):
    """
...
def finalize_production(conn, item, qty, inc_stock, user_id=None, username='WIP'):
    ...
    try:
        ...
        logger.error(f"Erro ao finalizar produção: {e}")
        raise

def register_loss(conn, item, stage, qty_loss, reason_loss):
    """
...
def update_priority(conn, item_id, increment):
    """
